        ...
        t(:div,{className: 'blogs_index container'},
          ...
          *splat_each(state.blogs) do |blog|
            t(:div, {key: "#{blog}", className: 'g_blog_box'},
              ...
              if blog.attributes[:published_at]
                t(:p, {}, 'published:' + Moment.new(blog.attributes[:published_at]).format('YYYY-MM-DD') )
              end,
              t(:p, {className: 'author'}, "author: #{blog.user.profile.name}"), 
              t(:div, {className: 'blog_content', dangerouslySetInnerHTML: {__html: blog.body}.to_n})
            )
          end,
          will_paginate
        )
