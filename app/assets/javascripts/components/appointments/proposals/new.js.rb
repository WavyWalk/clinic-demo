      ...
      class New < RW
        ...
        def component_did_mount
          Appointment.new_resource(component: self).then do |user|
            p '=-=-=-=-= -'
            p user.is_a?(User)
            p user.profile.try(:phone_number) == ""
            p !!user.profile.try(:phone_number)
            p "---------"
            begin
              if user.attributes[:unregistered]
                ...
                if (user.profile.try(:phone_number) == "" || !user.profile.try(:phone_number))
                  p 'no'
                  set_state no_contacts: true, loaded: true, step: 3, user_form_model: user
                else
                  p 'has'
                  set_state no_contacts: false, loaded: true
                end
              else
                set_state loaded: true
              end
              ...
              if state.step == 0
                t(:div, {},
                  ...
                  t(:div, {},
                    unless state.any_time_toggled
                      t(:span, {},
                        *splat_each(props.appointment_availabilities) do |k, v|
                          t(:div, {},
                            t(:p, {}, 'I want an appointment in these time windows with this doctor:'),
                    ...
                    t(:div, {className: 'extra_details'},
                      t(:p, {}, 'Write the specific time you want to visit, and please provide the reason for visit, e.g. general consultation'),
                      t(:p, {}, 'our staff will contact you and schedule for a wanted time'),
                ...
                t(:div, {className: 'guest_step'}, 
                  t(:div, {className: 'guest_appointment_form'}, 
                    ...
                    input(Forms::Input, state.user_form_model.profile, :phone_number, {show_name: 'your phone number'}),
