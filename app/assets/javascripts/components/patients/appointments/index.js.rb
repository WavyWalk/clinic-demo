    ...
    module Appointments
      ...
      class Week < RW
        ...
        def component_did_mount
          ...
          Appointment.index(component: self, payload: queries(props.date), namespace: 'patients').then do |response|
            users_with_appointments = response[0]
            pending_appointments = response[1]
            begin
            props.index.prepare_availability_tree(self, users_with_appointments)
            set_state pending_appointments: pending_appointments
            rescue Exception => e
              p e
            end
          end
        ...
        def render
          ...
          t(:div, {className: 'patient_appointments_index'},
            spinner,
            t(:div, {className: 'row'},
              ...
              if !state.pending_appointments.try(:empty?)
                t(:div, {className: 'unscheduled_proposals'},
                  t(:h3, {}, 'my unscheduled proposals'),
                  *splat_each(state.pending_appointments) do |p_a|
                    t(:p, {}, 'for date:  ' + Moment.new(p_a.attributes[:start_date]).format('YYYY-MM-DD'))
                  end
                )
              end
            ),
