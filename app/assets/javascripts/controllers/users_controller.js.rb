class UsersController < BaseController 
  ...
  def handle_signup_submit
    ...
    unless c.state.form_model.has_errors?
      c.state.form_model.sign_up(yield_response: true, payload: c.state.form_model.pure_attributes, component: c).then do |response|
        begin
        if (e = response.json[:user][:errors])
          ...
          c.create_flash('succesfully signed in')
      ...
      begin
        if x = response[:errors]
          ...
          if c.props.no_redirect
            ...
            Components::App::Router.history.pushState(nil, "/users/dashboard")
          end
          if c.props.on_login
